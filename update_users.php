...
if (isset($_GET['id'])) {
    $id = $_GET['id'];
    $sql = "SELECT * FROM `users` WHERE roll_no=$id";
    $result = mysqli_query($conn, $sql);
    $old_user = mysqli_fetch_assoc($result);

    $email_id = $old_user['email_id'];
    $user_name = $old_user['user_name'];
    $roll_no = $old_user['roll_no'];
    $department = $old_user['department'];
    $semester = $old_user['semester'];
}
...
<body>
    ...
    <div class="card" style="width: 30rem; margin:auto; padding: 10px;">
        <form action="update_users.php?id=<?php echo $id ?>" method="POST" id='form'>
